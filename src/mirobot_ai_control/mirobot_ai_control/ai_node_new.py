...
class MirobotAiNode(Node):
    ...
    #
    # [미검증] 위 표는 순기구학 계산일 뿐이다. 자기간섭(팔이 자기 몸체나
    # 베이스에 닿는지)은 계산으로 확인할 수 없고, 이 값은 아직 실기로 확인한
    # 적이 없다. 처음 쓰기 전에 반드시 GUI 의 수동 슬라이더로 J2 → J3 → J5
    # 순서로 천천히 맞춰 보고 닿는 곳이 없는지 눈으로 확인할 것.
    # 슬라이더가 토픽 발행보다 안전하다 — 천천히 움직이고 중간에 멈출 수 있다.
