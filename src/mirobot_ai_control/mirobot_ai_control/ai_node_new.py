...
class MirobotAiNode(Node):
    ...
    WS_Z_MIN_MM = 10.0
    WS_Z_MAX_MM = 340.0
    # J2 펌웨어 범위는 -35~+70도다. 원래는 여유 3도를 둔 67 로 목표점 단계에서
    # 막았으나, 그 한계에 걸려 실제로 쓰고 싶은 자세까지 못 가는 문제가 있어
    # 의도적으로 풀었다(200 은 사실상 "끔"이다 — J2 가 거기까지 갈 수 없다).
    #
    # 풀었을 때 남는 안전장치는 IK 뒤 관절각 클램프 하나뿐인데, 그 상한은
    # JOINT_SOFT_LIMITS[1] 의 +90 이라 펌웨어 +70 보다 느슨하다. 즉 70~90 구간
    # 에서는 보낸 각도와 실제 자세가 어긋날 수 있고, 그게 원래 이 상수가 막던
    # "튐"이다. 튐이 보이면 다른 데를 뒤지지 말고 여기(또는 위 소프트 리밋)를
    # 먼저 조일 것.
    WS_J2_MAX_DEG = 200.0
    WS_J2_MIN_DEG = -32.0
    ...
    # [주의] J2 하한은 실기로 확인한 값이다(2026-09-03). 예전에는 -60 이었는데
    # 실제 하드웨어는 -35 에서 멈춘다. 그 차이 때문에 -45 를 보내는 정리 자세가
    # 세 겹의 리밋(여기 / 브라우저 JOINT_LIMITS / robot_node JOINT_HARD_LIMITS)을
    # 전부 통과한 뒤 조용히 실패했다. 세 곳은 항상 같이 고칠 것.
    # 상한 +90 은 아직 확인하지 못했다 — 매뉴얼 기준은 +70 이라 같은 함정이
    # 남아 있을 수 있다. 슬라이더로 J2 를 올려 어디서 멈추는지 재 볼 것.
    JOINT_SOFT_LIMITS = [
        (-100.0, 100.0),  # J1
        ( -35.0,  90.0),  # J2  ← 하한 실측
        (-180.0,  50.0),  # J3
    ...
    #   theta   0° → 그리퍼 수직 아래
    #   theta -90° → 그리퍼 정면 수평
    # 아래 값은 theta = -35 + 35 + (-98) = -98 이다. 정면에서 8도 아래를 본다.
    # -90 을 정확히 맞추지 않은 이유는 J2 하한 때문이다(바로 아래 참고).
    #
    # 순기구학 (호밍 자세와 비교):
    #                  J2/J3/J5       툴끝 전방   손목 높이   J2 리밋 여유
    #   호밍 자세      0 /  0 / -90     288mm       255mm        35°
    #   이 자세      -35 / 35 / -98     227mm       236mm         0°
    #
    # [이전 값 -45/4/-49 는 도달 불가였다]
    # theta -90 은 만족했지만 J2 -45 가 하한 -35 를 10도 넘겨, 컨트롤러가
    # 명령을 받고도 자세를 만들지 못했다. 세 겹의 리밋이 전부 -60 이라 아무도
    # 걸러내지 못했고, 로그에는 발행 기록만 남아 원인을 찾기 어려웠다.
    # 판별법: robot_node 의 [ok측정] 왕복 지연을 본다. 도달 가능한 자세는
    # 29ms 에 ok 가 오지만, 도달 불가 자세는 537ms 에 눌러앉아 안 떨어진다.
    #
    # [실기 확인 완료 2026-09-03] GUI 수동 슬라이더로 이 값을 그대로 맞춰
    # 도달하는 것을 눈으로 확인했다. J6 는 0 으로 둔다 — 그리퍼가 돌아갈
    # 이유가 없고, 돌아가면 카메라 수평이 틀어진다.
    # 값을 바꿀 때는 같은 방법으로 다시 확인할 것. 자기간섭(팔이 자기 몸체나
    # 베이스에 닿는지)은 계산으로 확인할 수 없다. 슬라이더가 토픽 발행보다
    # 안전하다 — 천천히 움직이고 중간에 멈출 수 있다.
    #
    ...
    def _publish_stow_pose(self):
        ...
        [중계 경로 — AI 가 OFF 면 이 자세는 로봇에 닿지 않는다]
        이 토픽(ai_joint_commands)은 robot_node 가 직접 듣지 않는다. 브라우저의
        onAiJoints() 가 받아서 AI 가 ON 일 때만 joint_commands 로 넘긴다.

        모드 판정은 ai_enable 과 무관하게 돌아가므로, AI 가 OFF 인 채로도 BASE
        전환은 일어나고 이 함수도 호출된다. 그런데 그때 브라우저는 첫 줄에서
        곧장 return 하고, 게다가 수동 슬라이더 값(대개 전부 0)을 250ms 마다
        joint_commands 로 계속 내보내는 중이다. 즉 발행 로그는 남는데 팔은
        꿈쩍도 하지 않는다 — 실제로 2026-09-03 테스트에서 이 증상이 났다.

        그러니 "정리 자세가 안 먹는다" 면 로그부터 볼 것:
          grep -nE '모드로 전환|정리 자세|ai_enable' /tmp/petmate_logs/ai_node.log
        전환 시각과 AI ON 시각을 비교해 전환이 OFF 구간에 들어 있으면 그게
        원인이다. 코드 문제가 아니다.
