...
#
#  역할:
#    MediaPipe에 들어가기 "직전"의 프레임(좌우반전 포함, 오버레이 제외)을
...
class SessionRecorder:
    ...
    def __init__(self, session_dir, logger=None):
        ...
        self.meta = {
            ...
            # 첫 프레임이 들어올 때 실제 크기로 채워진다(아래 submit 참고).
            # 하드코딩하면 파이프라인이 바뀌었을 때 메타가 조용히 거짓말을 한다.
            "frame_size": None,
    ...
    def submit(self, frame_bgr, src_t=-1.0):
        ...
        # ── 실제 프레임 크기 기록 (첫 프레임에서 1회만) ────────────────────
        # 클라이언트/서버 설정에 따라 480×480일 수도 640×480일 수도 있으므로
        # 추측하지 않고 들어온 프레임에서 직접 읽는다. 나중에 세션별 종횡비를
        # 추적하려면 이 값이 정확해야 한다.
        if self.meta.get("frame_size") is None:
            h0, w0 = frame_bgr.shape[:2]
            self.meta["frame_size"] = [int(w0), int(h0)]
            self.meta["aspect"] = round(w0 / h0, 4)
            self._write_meta()
            self._log(f"[녹화] 프레임 크기 {w0}×{h0} (종횡비 {w0/h0:.3f})")
